Q.6 Get department wise maximum salary from employee table order by 
...
```sql
SELECT Department, MAX(Salary) AS Max_Salary FROM employee GROUP BY Department ORDER BY Department;
```
output:
